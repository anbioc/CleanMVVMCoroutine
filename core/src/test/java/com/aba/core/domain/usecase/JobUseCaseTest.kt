...
class JobUseCaseTest {
    ...
    private suspend fun givenResultIsSuccessful() {
        given(mockRepository.getJobs(SOME_DESCRIPTION, SOME_LOCATION)).willReturn(ResultResponse.Success(
            SOME_JOB_DOMAIN_MODELS))
    }

    private suspend fun givenResultIsFailure() {
        given(mockRepository.getJobs(SOME_DESCRIPTION, SOME_LOCATION)).willReturn(ResultResponse.Failure(
            Throwable()
        ))
    }
    ...
    private suspend fun whenGetJobs() {
        result = subject.executeAsync(JOB_USE_CASE_PARAMS)
    }

    /*
     * Then
     */
    private fun thenResultIsValid() = with((result as ResultResponse.Success<List<JobDomainModel>>).data) {
